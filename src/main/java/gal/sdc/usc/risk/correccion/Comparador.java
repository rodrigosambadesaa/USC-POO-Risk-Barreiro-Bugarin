...
public final class Comparador {
  ...
  private void success(String o) {
    append(
        "["
            + Ajustes.Colores.VERDE.getFondo()
            + Ajustes.Colores.NEGRO.getTexto()
            + "OK"
            + Ajustes.Colores.RESET
            + "] "
            + Ajustes.Colores.VERDE.getTexto()
            + o
            + Ajustes.Colores.RESET);
  }
  ...
  private void warn(String o) {
    append(
        "["
            + Ajustes.Colores.AMARILLO.getFondo()
            + Ajustes.Colores.NEGRO.getTexto()
            + "WARN"
            + Ajustes.Colores.RESET
            + "] "
            + Ajustes.Colores.AMARILLO.getTexto()
            + o
            + Ajustes.Colores.RESET);
  }
  ...
  private void danger(String o) {
    append(
        "["
            + Ajustes.Colores.ROJO.getFondo()
            + Ajustes.Colores.NEGRO.getTexto()
            + "ERROR"
            + Ajustes.Colores.RESET
            + "] "
            + Ajustes.Colores.ROJO.getTexto()
            + o
            + Ajustes.Colores.RESET);
  }
  ...
  private void comando(String o) {
    append(
        Ajustes.Colores.AMARILLO.getTexto()
            + "$> "
            + Ajustes.Colores.RESET
            + Ajustes.Colores.VIOLETA.getTexto()
            + o
            + Ajustes.Colores.RESET);
  }
  ...
  private void puntos(int max, int puntos) {
    ...
    append(
        Ajustes.Colores.VIOLETA.getTexto()
            + ">> "
            + Ajustes.Colores.RESET
            + Ajustes.Colores.CELESTE.getTexto()
            + puntos
            + " / "
            + max
            + " = "
            + (int) (((float) puntos / max) * 100)
            + "%"
            + Ajustes.Colores.RESET);
  }
  ...
  private void comparar() {
    if (alumno.getComandos().size() != profesor.getComandos().size()) {
      warn(
          "El número de comandos en los archivos no coincide, es posible que aparezcan discrepancias.");
    }
  ...
  public static String imprimirError(Object r1, Object r2) {
    ...
    String s1 = String.format("[%2d/%2d] %s%n", 1, 1, r1.toString());
    String s2 = String.format("[%2d/%2d] %s", eq ? 1 : 0, 1, r2.toString());
    return Ajustes.Colores.AZUL.getTexto()
        + s1
        + (eq ? Ajustes.Colores.VERDE.getTexto() : Ajustes.Colores.AMARILLO.getTexto())
        + s2
        + Ajustes.Colores.RESET;
  }
