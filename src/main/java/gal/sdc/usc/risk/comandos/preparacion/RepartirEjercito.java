...
public class RepartirEjercito extends Partida implements IComando {
  ...
  public void ejecutar(String[] comandos) {
    ...
    Integer asignado =
        paisFinal.getEjercito().recibir(paisFinal.getJugador().getEjercitosPendientes(), numero);
    if (asignado != null) {
      ...
      List<Pais> paisesJugadorContinente =
          super.getMapa().getPaisesPorJugador(paisFinal.getJugador()).stream()
      ...
      for (Pais paisOcupado : paisesJugadorContinente) {
        paisesOcupadosContinente.add(
            new SalidaDupla(paisOcupado.getNombre(), paisOcupado.getEjercito()));
      }
  ...
  private void comprobarEjercitos(Jugador jugador) {
    if (super.isJugando()) {
      ...
      if (jugador.getEjercitosPendientes().toInt() == 0) {
        // super.moverTurno();
        // Resultado.out("[" + new Colores(super.getJugadorTurno().getNombre(),
        // super.getJugadorTurno().getColor()) + "] Repartiendo ejércitos...");
