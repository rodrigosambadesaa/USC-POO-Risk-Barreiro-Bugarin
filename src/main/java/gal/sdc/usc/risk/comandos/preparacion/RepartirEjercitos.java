...
public class RepartirEjercitos extends Partida implements IComando {
  ...
  public void ejecutar(String[] comandos) {
    ...
    if (super.getMapa().getPaisesPorCeldas().values().stream()
        .anyMatch(p -> p.getJugador() == null)) {
  ...
  private void parsearR2R5(HashMap<Continente, List<Jugador>> jugadoresContinentes) {
    for (Map.Entry<Continente, List<Jugador>> jugadorContinente : jugadoresContinentes.entrySet()) {
      for (Jugador jugador : jugadorContinente.getValue()) {
        ...
        for (Map.Entry<Continente, List<Jugador>> jugadorContinente2 :
            jugadoresContinentes.entrySet()) {
    ...
    for (Map.Entry<Jugador, List<Continente>> e : jugadoresDuplicados.entrySet()) {
      ...
      for (Continente continente : e.getValue()) {
        if (maxOcupacion.size() == 0) {
          ...
          if ((maxOcupacion.get(0).getPaisesPorJugador(e.getKey()).size()
                  / maxOcupacion.get(0).getPaises().size())
              == (continente.getPaisesPorJugador(e.getKey()).size()
                  / continente.getPaises().size())) {
            maxOcupacion.add(continente);
          } else if (maxOcupacion.get(0).getPaisesPorJugador(e.getKey()).size()
                  / maxOcupacion.get(0).getPaises().size()
              < (continente.getPaisesPorJugador(e.getKey()).size())
                  / continente.getPaises().size()) {
            maxOcupacion.clear();
      ...
      if (maxOcupacion.size() == 0) {
        ...
        for (Continente continente : maxOcupacion) {
          if (minFronteras == null
              || minFronteras.getPaisesFrontera().size() > continente.getPaisesFrontera().size()) {
  ...
  private void repartirR1R2R4R5() {
    ...
    for (Map.Entry<Continente, List<Jugador>> e : this.continentesR1.entrySet()) {
      ...
      if (continente.equals(super.getMapa().getContinentePorNombre(Continentes.OCEANIA.getNombre()))
          || continente.equals(
              super.getMapa().getContinentePorNombre(Continentes.AMERICASUR.getNombre()))) {
        factor = 1.5;
      } else {
        factor = 1;
      }
      ...
      for (Jugador jugador : jugadores) {
        numEjercitos =
            Math.toIntExact(
                Math.round(
                    jugador.getEjercitosPendientes().toInt()
                        / (factor * continente.getPaisesPorJugador(jugador).size())));
      ...
      for (Jugador jugador : jugadores) {
        numEjercitos =
            Math.toIntExact(
                Math.round(
                    jugador.getEjercitosPendientes().toInt()
                        / (2.0 * continente.getPaisesPorJugador(jugador).size())));
        for (Pais pais : continente.getPaisesPorJugador(jugador)) {
          RepartirEjercitos.repartir(pais, super.getJugadorTurno(), numEjercitos);
        }
      }
      ...
      for (Pais pais : jugador.getPaises()) {
        numEjercitos =
            Math.toIntExact(
                Math.round(
                    jugador.getEjercitosPendientes().toInt()
                        / (factor * pais.getContinente().getPaisesPorJugador(jugador).size())));
        RepartirEjercitos.repartir(pais, super.getJugadorTurno(), numEjercitos);
      }
    }
  }
  ...
  private void repartirR3R6R8() {
    for (List<Jugador> jugadores : this.continentesR1.values()) {
      for (Jugador jugador : jugadores) {
        List<Pais> paises = new ArrayList<>(jugador.getPaises());
        paises.sort(
            Comparator.comparingInt((Pais p) -> p.getFronteras().getTodas().size()).reversed());
