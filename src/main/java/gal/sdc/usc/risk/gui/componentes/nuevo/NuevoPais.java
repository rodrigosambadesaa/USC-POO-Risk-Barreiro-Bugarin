...
/** Diálogo heredado del editor experimental de mapas. */
public class NuevoPais extends Partida {
  private final int x;
  private final int y;

  public static void generarDialogo(int x, int y) {
    new NuevoPais(x, y).generar();
  }

  private NuevoPais(int x, int y) {
    this.x = x;
    this.y = y;
  }
  ...
  private List<Label> subequipamientosDisponibles(Label paisLabel) {
    ...
    for (Carta carta : super.getCartasMonton()) {
      ...
      subequipamientos.put(
          carta.getEquipamiento().getNombre() + " / " + carta.getSubEquipamiento().getNombre(),
          carta.getSubEquipamiento());
    }
  ...
  private void generar() {
    Dialogo dialogo = new Dialogo().setHeading("Seleccionar Carta").setCloseText("Cancelar");
    ...
    comboSubequipamientos
        .disableProperty()
        .bind(comboPaises.getSelectionModel().selectedItemProperty().isNull());

    comboPaises.setPrefWidth(Float.MAX_VALUE);
    comboPaises.getItems().addAll(continentesDisponibles());
