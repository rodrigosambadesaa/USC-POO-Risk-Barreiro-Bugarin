...
public class NuevoAtaque extends Partida {
  ...
  private void generar() {
    Dialogo dialogo = new Dialogo().setHeading("Atacar Paises").setCloseText("Cancelar");
    ...
    fila1
        .getChildren()
        .add(Utils.labelColor(pais1.getNombre(), pais1.getJugador().getColor().getHex()));
    ...
    fila2
        .getChildren()
        .add(Utils.labelColor(pais2.getNombre(), pais2.getJugador().getColor().getHex()));
    ...
    ejecutar.setOnAction(
        event -> {
          ...
          for (int i = 0; i < atacantes; i++) {
            atacante.add(
                Integer.toString(gal.sdc.usc.risk.application.GameContext.current().dice().roll()));
          }
              ...
              new EjecutorListener() {
                ...
                public void onComandoEjecutado() {
                  ...
                  PrincipalController.mensaje(
                      "Se han tirado los dados "
                          + String.join("x", atacante)
                          + " ["
                          + pais1.getNombre()
                          + "] frente a "
                          + String.join("x", defensor)
                          + " ["
                          + pais2.getNombre()
                          + "]");
                }
