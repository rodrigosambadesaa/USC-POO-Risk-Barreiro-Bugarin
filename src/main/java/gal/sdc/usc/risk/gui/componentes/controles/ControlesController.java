...
public class ControlesController extends Partida {
  @FXML private Pane contenedorComandos;
  @FXML private AnchorPane anchor;
  @FXML public JFXToggleButton toogleIndirectas;
  @FXML public VBox contenedorJugador;
  ...
  public void actualizarComandos(Scene scene) {
    ...
    if (!super.isJugando()) {
      ...
      if (super.getComandos().getLista().contains(CambiarCartas.class)) {
        ...
        cambiar.setDisable(
            MapaController.isAtacar() || MapaController.isRearmar() || MapaController.isRepartir());
                ...
                new EjecutorAccion() {
                  ...
                  public void onClick(Object o) {
                    if (MapaController.cambiarRepartir()) {
                      PrincipalController.mensaje(
                          "Clica en el país para asignar un ejército, o mantén pulsado para "
                              + "asignar más de uno. Para cancelar, pulsa de nuevo el botón de Repartir.",
                          10);
                      MapaController.setAccion(
                          new EjecutorAccion() {
                            @Override
                            public void onClick(Object o) {
                              Ejecutor.comando(
                                  "repartir ejercito 1 " + ((Pais) o).getAbreviatura(), null);
                            }
  ...
  public void actualizarJugador(Scene scene) {
    ...
    if (this.simularJugando()) {
      ...
      button.setStyle(
          button.getStyle()
              + "-fx-text-fill: "
              + super.getJugadorTurno().getColor().getHex()
              + "; "
              + "-fx-border-width: 3; "
              + "-fx-border-radius: 5; "
              + "-fx-border-color: "
  ...
  private JFXButton crearPreControl(String nombre, String icon, EjecutorAccion accion) {
    ...
    Label icono = new Label(icon);
    icono.setStyle("-fx-font-size: 24px; -fx-font-weight: bold;");
  ...
  private void initialize() {
    anchor
        .getStylesheets()
        .add(ControlesController.class.getResource("controles.css").toExternalForm());
  }
  ...
  public void comandos() {
    ...
    List<Class<? extends IComando>> comandos =
        new java.util.ArrayList<>(super.getComandos().getLista());
    ...
    for (Class<? extends IComando> comando : comandos) {
      ...
      IComando comandoObject = super.getRegistry().create(comando);
      Label label = new Label(comandoObject.nombre());
    ...
    comboComando
        ...
        .addListener(
            (o, oldV, newV) -> {
              ...
              errorContenedor.setVisible(false);
              errorContenedor.setManaged(false);
              Class<? extends IComando> comando =
                  super.getComandos().getLista().stream()
                      .filter(type -> type.getName().equals(newV.getId()))
                      .findFirst()
                      .orElseThrow(
                          () ->
                              new IllegalStateException("Comando no registrado: " + newV.getId()));
              IComando comandoObj = super.getRegistry().create(comando);
    ...
    ejecutar.setOnAction(
        event -> {
          errorContenedor.setVisible(false);
          errorContenedor.setManaged(false);
          Ejecutor.comando(
              comandoTexto.getText(),
              new EjecutorListener() {
                @Override
                public void onComandoError(ExcepcionRISK e) {
