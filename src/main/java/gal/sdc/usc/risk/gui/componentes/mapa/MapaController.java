...
import static gal.sdc.usc.risk.domain.Mapa.MAX_PAISES_X;
import static gal.sdc.usc.risk.domain.Mapa.MAX_PAISES_Y;
...
public class MapaController extends Partida {
  ...
  private static final HashMap<Button, Animation> animacionesSeleccion = new HashMap<>();
  private static final HashMap<Button, EventHandler<MouseEvent>> handlersBotones = new HashMap<>();
  private static final List<Pais> paisesSeleccionados = new LinkedList<>();
  ...
  @FXML public VBox contenedor;
  @FXML private AnchorPane anchor;

  public MapaController() {}

  // Estado del editor experimental de mapas conservado por compatibilidad.
  ...
  public static void cambiarCreando() {
    ...
    if (creando) {
      ...
      for (Continentes continente : Continentes.values()) {
        Continente.Builder preContinente =
            new Continente.Builder(continente)
  ...
  public void actualizarFronteras(Scene scene) {
    ...
    for (EnlacesMaritimos enlace : EnlacesMaritimos.values()) {
      ...
      JFXButton button1 =
          (JFXButton) scene.lookup("#" + pais1.getCelda().getY() + "-" + pais1.getCelda().getX());
      JFXButton button2 =
          (JFXButton) scene.lookup("#" + pais2.getCelda().getY() + "-" + pais2.getCelda().getX());
      ...
      ObjectBinding<Double> startX =
          Bindings.createObjectBinding(
              ...
              button1.localToSceneTransformProperty(),
              button2.localToSceneTransformProperty(),
              button1.widthProperty());
      ObjectBinding<Double> startY =
          Bindings.createObjectBinding(
              () -> {
                double x1 = button1.localToScene(new Point2D(0, 0)).getX();
                double x2 = button2.localToScene(new Point2D(0, 0)).getX();
              ...
              button1.localToSceneTransformProperty(),
              button2.localToSceneTransformProperty(),
              button1.heightProperty());
      ObjectBinding<Double> endX =
          Bindings.createObjectBinding(
              () -> {
                double x1 = button1.localToScene(new Point2D(0, 0)).getX();
                double x2 = button2.localToScene(new Point2D(0, 0)).getX();
              ...
              button1.localToSceneTransformProperty(),
              button2.localToSceneTransformProperty(),
              button2.widthProperty());
      ObjectBinding<Double> endY =
          Bindings.createObjectBinding(
              () -> {
                double x1 = button1.localToScene(new Point2D(0, 0)).getX();
                double x2 = button2.localToScene(new Point2D(0, 0)).getX();
    ...
    for (int i = 0; i < MAX_PAISES_Y; i++) {
      for (int j = 0; j < MAX_PAISES_X; j++) {
        ...
        if (paises.containsKey(celda)) {
          ...
          if (pais.getJugador() != null) {
            button.setStyle(
                button.getStyle()
                    ...
                    + pais.getJugador().getColor().getHex()
                    + " 0%,"
                    + pais.getJugador().getColor().getHex()
                    + " 49%,"
                    + "derive("
          ...
          String bordesContinente =
              fronterasContinente(
                  norte == null || !norte.getContinente().equals(pais.getContinente()),
