...
public class PrincipalController extends Application {
  ...
  @FXML public HBox contenedorVertical;
  @FXML public VBox mapa;
  @FXML public VBox controlesVertical;
  @FXML public VBox controlesHorizontal;
  ...
  private static void mensaje(JFXSnackbarLayout layout, Integer duracion) {
    JFXSnackbarLayout finalLayout =
        new JFXSnackbarLayout(layout.getToast(), "Cerrar", e -> snackbar.close());
    if (duracion != null) {
  ...
  @Override
  public void init() {
    game = new Game(new ConsolaNormal());
  }
  ...
  public void initialize() {
            ...
            ((observable, oldValue, newValue) -> {
              ...
              try {
                if (newValue.doubleValue() >= pequenoInt && oldValue.doubleValue() < pequenoInt) {
                  ...
                  ((ControlesController) loader.getController())
                      .actualizarComandos(contenedor.getScene());
                  ((ControlesController) loader.getController())
                      .actualizarJugador(contenedor.getScene());
                } else if (newValue.doubleValue() < pequenoInt
                    && oldValue.doubleValue() >= pequenoInt) {
                  controlesVertical.setVisible(false);
                  controlesHorizontal.setVisible(true);
                  loader.load(
                      ControlesController.class.getResource("horizontal.fxml").openStream());
                  ((ControlesController) loader.getController())
                      .actualizarComandos(contenedor.getScene());
                  ((ControlesController) loader.getController())
                      .actualizarJugador(contenedor.getScene());
    ...
    FloatBinding contenedorVerticalHeight =
    ...
    FloatBinding controlesVerticalWidth =
        Bindings.createFloatBinding(
            () -> {
              if (pequeno.get()) {
    ...
    FloatBinding controlesHorizontalHeight =
        Bindings.createFloatBinding(
            () -> {
              if (pequeno.get()) {
  ...
  private void tipoMapa() {
    Dialogo dialogo = new Dialogo().setHeading("Bienvenido").setCloseText("Mapa Clásico");

    Label label =
        new Label(
            "Para empezar, puedes escoger si quieres crear un mapa personalozado o usar el "
    ...
    button.setOnAction(
        e -> {
          ...
          PrincipalController.mensaje(
              "Todavía en progreso... Pulsa el botón de arriba a la derecha.");
          dialogo.close();
        });
